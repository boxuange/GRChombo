...
////////////////////////////////////////////////////////////////////////////////
  ...
  Star1.StarZ = coords.z - m_params.centerSF[2];
...
////////////////////////////////////////////////////////////////////////////////
  ...
  FOR2(i,j){
    h_cartesian[i][j] = Star1.h_cartesian[i][j] + Star2.h_cartesian[i][j];
    K_cartesian[i][j] = Star1.K_cartesian[i][j] + Star2.K_cartesian[i][j];
  }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
template <class data_t>
void Oscilloton::Spawn(StarData &DeStar)const{
// List of declare
...
// Calculate variables from the data file
  if(floor(rr/m_spacing)<m_row_max-1){
    ...
    phi_components[0] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,phi1);
    phi_components[1] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,phi3);
    phi_components[2] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,phi5);
    phi_components[3] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,phi7);
    phi_components[4] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,phi9);
    phi_components[5] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,phi11);

    A_components[0] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,a0);
    A_components[1] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,a2);
    A_components[2] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,a4);
    A_components[3] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,a6);
    A_components[4] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,a8);
    A_components[5] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,a10);
    A_components[6] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,a12);

    C_components[0] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,c0);
    C_components[1] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,c2);
    C_components[2] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,c4);
    C_components[3] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,c6);
    C_components[4] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,c8);
    C_components[5] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,c10);
    C_components[6] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,c12);

    d_phi_dr_components[0] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_phi1_dr);
    d_phi_dr_components[1] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_phi3_dr);
    d_phi_dr_components[2] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_phi5_dr);
    d_phi_dr_components[3] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_phi7_dr);
    d_phi_dr_components[4] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_phi9_dr);
    d_phi_dr_components[5] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_phi11_dr);

    d_A_dr_components[0] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_a0_dr);
    d_A_dr_components[1] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_a2_dr);
    d_A_dr_components[2] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_a4_dr);
    d_A_dr_components[3] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_a6_dr);
    d_A_dr_components[4] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_a8_dr);
    d_A_dr_components[5] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_a10_dr);
    d_A_dr_components[6] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_a12_dr);

    d_C_dr_components[0] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_c0_dr);
    d_C_dr_components[1] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_c2_dr);
    d_C_dr_components[2] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_c4_dr);
    d_C_dr_components[3] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_c6_dr);
    d_C_dr_components[4] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_c8_dr);
    d_C_dr_components[5] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_c10_dr);
    d_C_dr_components[6] = linear_interpolation_new<double>(DeStar.vx,t,x,y,z,d_c12_dr);
    ...
    for(int i=0; i<6;i++){
      ...
      d_phi_dx[0]  += d_phi_dr_components[i]*dr_dx[0]*cos((2*i+1)*m_omega*tboost) + phi_components[i]*(2*i+1)*m_omega*m_gamma*DeStar.vx*sin((2*i+1)*m_omega*tboost);
...
// Repeat as above but beyond the data files
    ...
    d_grr_dt = - pow(grr,2) * (m_rs_g_rr/(pow(rr,2))) * dr_dt; //((1.)/(m_c-m_rs_g_rr/rr))*((1.)/(m_c-m_rs_g_rr/rr)) * m_rs_g_rr/pow(rr,3)* m_gamma * DeStar.vx * x;
    lapse_unboosted = sqrt(m_b - m_rs_alpha/rr);
    d_lapse_dr = m_rs_alpha/pow(rr,2) * (1./(2.*lapse_unboosted));
    d_lapse_dt = (1./(2.*lapse_unboosted)) * (m_rs_alpha/pow(rr,2)) * dr_dt; //m_rs_alpha/pow(rr,3) * m_gamma * DeStar.vx * x * (1./(2.*lapse_unboosted));
...
// Calculate lapse (now boosted)
  DeStar.lapse = pow(pow(paramgamma,2)*pow(-1 + pow(vel,2),2)*pow(pow(lapseunboosted,-2) - pow(aaa,-1)*pow(paramgamma,2)*pow(vel,2)*pow(-(t*vel) + x,2)*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),-1) -
                                                                  pow(vel,2)*(pow(y,2) + pow(z,2))*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),-1),-1),0.5);

  DeStar.beta[0] = vel*pow(paramgamma,2)*(pow(lapseunboosted,2) - (aaa*pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2))*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),-1));
  DeStar.beta[1] = (-1 + aaa)*vel*(t*vel - x)*y*pow(paramgamma,2)*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),-1);
  DeStar.beta[2] = (-1 + aaa)*vel*(t*vel - x)*z*pow(paramgamma,2)*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),-1);
  ...
  DeStar.h_cartesian[0][0] = pow(paramgamma,2)*(-(pow(lapseunboosted,2)*pow(vel,2)) + (aaa*pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2))*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),-1));
  DeStar.h_cartesian[0][1] = (-1 + aaa)*(-(t*vel) + x)*y*pow(paramgamma,2)*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),-1);
  DeStar.h_cartesian[0][2] = (-1 + aaa)*(-(t*vel) + x)*z*pow(paramgamma,2)*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),-1);
  DeStar.h_cartesian[1][1] = (pow(paramgamma,2)*pow(-(t*vel) + x,2) + aaa*pow(y,2) + pow(z,2))*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),-1);
  DeStar.h_cartesian[2][2] = (pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + aaa*pow(z,2))*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),-1);
  DeStar.h_cartesian[1][2] = (-1 + aaa)*y*z*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),-1);
  ...
  DeStar.K_cartesian[0][0] = -(pow(paramgamma,3)*(-1 + pow(vel,2))*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),-1.5)*
  ...
  DeStar.K_cartesian[0][1] = y*pow(paramgamma,2)*(-1 + pow(vel,2))*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),-1.5)*
  ...
  DeStar.K_cartesian[0][2] = z*pow(paramgamma,2)*(-1 + pow(vel,2))*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),-1.5)*
                             (-(paramgamma*vel*(t*vel - x)*pow(lapseunboosted,2)*(daaadr*paramgamma*(t*vel - x)*(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2)) +
                                                                                  (2*paramgamma*(t*vel - x) + daaadt*vel*(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2)))*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),0.5))) +
                              aaa*(paramgamma*(t*vel - x)*(2*paramgamma*vel*(t*vel - x)*pow(lapseunboosted,2) + daaadt*(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2)))*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),0.5) +
                                   2*daldr*lapseunboosted*vel*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),2)))*
  ...
  DeStar.K_cartesian[1][1] = paramgamma*(-1 + pow(vel,2))*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),-1.5)*
  ...
  DeStar.K_cartesian[1][2] = -(paramgamma*y*z*(-1 + pow(vel,2))*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),-1.5)*
  ...
  DeStar.K_cartesian[2][2] = paramgamma*(-1 + pow(vel,2))*pow(pow(paramgamma,2)*pow(-(t*vel) + x,2) + pow(y,2) + pow(z,2),-1.5)*
  ...
  FOR2(i,j){
    DeStar.K_cartesian[i][j]=1.0/(2.0*DeStar.lapse) * DeStar.K_cartesian[i][j];
  }
...
template <class data_t>
//double Oscilloton::linear_interpolation_2022117(Coordinates<data_t> coords, const double (*data_jt)) const {
double Oscilloton::linear_interpolation_new(double vx, double t, double x, double y, double z, const double (*data_jt)) const {
